 ...
 */
public class ViterbiCalculatorWithAttribute
{	
	/*
	...
	ViterbiCalculatorWithAttribute(List<? extends O> oseq, Hmm<O> hmm, Hashtable<Integer, AttributeDataSequences> attributeSet, StateAttributeSequence[][] transitionAttributeSet, int idNum, List<StateAttributeSequence> stateAttributeSet)
	{
		if (oseq.isEmpty())
		...
		while (oseqIterator.hasNext()) {
			...
			for (int i = 0; i < hmm.nbStates(); i++)
				computeStep(hmm, observation, t, i, attributeSequence, transitionAttributeSet, stateAttributeSet);
