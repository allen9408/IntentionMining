...
public class ForwardBackwardScaledCalculator extends ForwardBackwardCalculator {
	...
	private static double actSum;
	private static double actNum;
	...
	public <O extends Observation> ForwardBackwardScaledCalculator(List<? extends O> oseq, Hmm<O> hmm,
			EnumSet<Computation> flags, double actSum2) {
		this.actSum = actSum2;
		this.actNum = oseq.size();
		if (oseq.isEmpty())
			throw new IllegalArgumentException();

		ctFactors = new double[oseq.size()];
		Arrays.fill(ctFactors, (double) 0.0);
	 ...
	 */
	public <O extends Observation> ForwardBackwardScaledCalculator(List<? extends O> oseq, Hmm<O> hmm) {
		this(oseq, hmm, EnumSet.of(Computation.ALPHA), actSum);
	}
	...
	private void scale(double[] ctFactors, double[][] array, int t) {
		...
		for (int i = 0; i < table.length; i++) {
			sum += table[i];
//			System.out.println("table " + 1 + ": " + table[i]);
		}
			

		ctFactors[t] = sum;
//		System.out.println("ctFactor = " + sum);
		for (int i = 0; i < table.length; i++)
			// if (sum != 0)
			table[i] /= sum;
	}
	...
	private <O extends Observation> void computeProbability(List<O> oseq,
		...
		double lnPro[] = new double[oseq.size()];
		// System.out.println("ctFactor: ");
		for (int t = 0; t < oseq.size(); t++) {
			lnProbability += Math.log(ctFactors[t]);
			lnPro[t] = Math.log(ctFactors[t]);
			// System.out.println(ctFactors[t] + " ");
			// System.out.println("t: " + t + " c: " + Math.log(ctFactors[t]));
		}
//		System.out.println();
//		System.out.println("transition ln Probability: ");
//		for (int i = 0; i < oseq.size(); i++) {
//			System.out.print(df.format(lnPro[i]) + " ");
//		}
//		System.out.println();
	...
	public double lnProbability() {
		// System.out.println("return pro:" + lnProbability);
