...
import HMM.Observation.CentroidFactory;
...
{	
	...
	private void learnEpi(Hmm<?> hmm) {
		double[] epi = new double[nbStates];
		...
		for (List<? extends O> sequence : obsSeqs) {
			epi[clusters.clusterNb(sequence.get(sequence.size() - 1))]++;
		}
		
		for (int i = 0; i < nbStates; i++) {
			hmm.setEpi(i, epi[i] / obsSeqs.size());
		}
	}
