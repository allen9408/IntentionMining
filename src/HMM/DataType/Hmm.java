...
import HMM.PDF.Opdf;
import HMM.PDF.OpdfFactory;
import HMM.PathFindingAlgorithm.ForwardBackwardCalculator;
...
public class Hmm<O extends Observation> implements Serializable, Cloneable {
	 ...
	 * @param randomNumber
	 */
	public Hmm(int nbStates, OpdfFactory<? extends Opdf<O>> opdfFactory, boolean random) {
		if (nbStates <= 0)
			throw new IllegalArgumentException("Number of states must be " + "strictly positive");
	...
	public double getEpi(int stateNB) {
		return epi[stateNB];
	}
	...
	public void setEpi(int stateNb, double value) {
		epi[stateNb] = value;
	}
	...
	public ArrayList<Opdf<O>> getOpdf() {
		return opdfs;
	}
	 ...
	 * @author Sen
	 */
	public void setA(double[][] transitionMatrix) {
		a = transitionMatrix;
	}
	 ...
	 * 
	 */
	public double lnProbability_MDL(List<? extends O> oseq, int d) {
		Hmm<O> MDLHmm = new Hmm<O>(this.nbStates());
		try {
			MDLHmm = this.clone();
		...
		double[][] A_MDL = this.getA();
		/* round the real-value transition probability */
		for (int i = 0; i < A_MDL.length; i++) {
			double rowSum = 0;
			for (int j = 0; j < A_MDL.length; j++) {
				A_MDL[i][j] = round(A_MDL[i][j], d);
				rowSum += A_MDL[i][j];
			}
			/* renormalize the data */
			if (0 != rowSum) {
				for (int j = 0; j < A_MDL.length; j++) {
					A_MDL[i][j] = A_MDL[i][j] / rowSum;
				}
			}
		}
//		System.out.println();
		MDLHmm.setA(A_MDL);
		/* also needs to round the observation probability */
		/*
		 * but since right now, we only allows one symbol per state. Thus we
		 * skip this for now
		 */

		return (new ForwardBackwardScaledCalculator(oseq, MDLHmm)).lnProbability();
	}

	/**
	 * help function for lnProbability_MDL
	 * 
	 * @author Sen
	 */
